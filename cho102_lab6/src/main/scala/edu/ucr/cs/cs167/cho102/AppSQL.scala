    ...
    try {
      ...
      command match {
        case "count-all" =>
          ...
          val query: String = "SELECT count(*) FROM log_lines;"// TODO 9: write SQL query to count the total number of rows in `log_lines`
        ...
        case "code-filter" =>
          ...
          val query: String = "SELECT count(*) FROM log_lines WHERE response=" + responseCode + ";"// TODO 10: write SQL query to filter view `log_lines` by `responseCode`
          val count: Long = spark.sql(query).first().getAs[Long](0)
        ...
        case "time-filter" =>
          ...
          val query: String = "SELECT count(*) FROM log_lines WHERE time BETWEEN " + from + " AND " + to + ";"// TODO 11: write SQL query to filter view `log_lines` by `time` between `from` and `to`
          val count: Long = spark.sql(query).first().getAs[Long](0)
        ...
        case "count-by-code" =>
          ...
          println("Code,Count")
          val query: String = "SELECT response, COUNT(bytes) FROM log_lines GROUP BY response ORDER BY response;"// TODO 12: write SQL query the get the count for each `response`, results must be ordered by `response` in ascending order
        ...
        case "sum-bytes-by-code" =>
          ...
          val query: String = "SELECT response, SUM(bytes) FROM log_lines GROUP BY response ORDER BY response;"// TODO 13: write SQL query the get the sum of `bytes` for each `response`, results must be ordered by `response` in ascending order
            spark.sql(query).foreach(row => println(s"${row.get(0)},${row.get(1)}"))
        case "avg-bytes-by-code" =>
          ...
          val query: String = "SELECT response, AVG(bytes) FROM log_lines GROUP BY response ORDER BY response;"// TODO 14: write SQL query the get the average of `bytes` for each `response`, results must be ordered by `response` in ascending order
            spark.sql(query).foreach(row => println(s"${row.get(0)},${row.get(1)}"))
        case "top-host" =>
          ...
          val query: String = "SELECT host, COUNT(*) AS cnt FROM log_lines GROUP BY host ORDER BY cnt DESC LIMIT 1;"// TODO 15: write SQL query the get the `host` with the largest count
